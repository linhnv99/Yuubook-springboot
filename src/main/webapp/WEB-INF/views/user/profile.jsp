				...
				<div class="row">
					...
					<div class="col-9 pl-4">
						<div class="mainbox2-body py-2 px-3">
							...
							<div class="box-info pl-3">
								<p>Họ tên: <span> ${user.surname } ${user.name }</span> </p>
								<p>Email: <span> ${user.email } </span> </p>
								<p>Số điện thoại: <span> ${user.phone != '' && user.phone!=null ? user.phone : 'không xác định' } </span> </p>
							</div>
							<div class="box-address pl-3">
								<h6>ĐỊA CHỈ GIAO HÀNG MẶC ĐỊNH <a href="${path }/profile/address">(Sửa)</a></h6>
								<c:if test="${userAddress.id !=null }">
									<p>${userAddress.addressDetail }, ${userAddress.wards } - ${userAddress.district } - ${userAddress.province }</p>
								</c:if>
								<c:if test="${userAddress.id == null }">
									<p>Không xác định</p>
								</c:if>
