						...
						<div class="mainbox2-body py-2 px-3">
							...
							<div class="box-order">
								<c:choose>
												...
												<c:forEach items="${orders }" var="order">
													<tr>
														...
														<td><c:if test="${order.orderStatus == 0 }">
																...
																<span class="bg-info status"> Đang giao hàng </span>
															</c:if> <c:if test="${order.orderStatus == 3 }">
																<span class="bg-success status"> Hoàn thành</span>
															</c:if>
														<td>
															<div class="action">
																<a class="btn btn-info" data-toggle="modal"
																	data-target="#order${order.id }">Xem</a> <a
									...
									<c:otherwise>
										<p class="text-center my-4">Bạn chưa có đơn hàng nào</p>
									</c:otherwise>
								</c:choose>

								<div class="text-right mt-3 mb-2">
									<a href="${path }/profile/order/deleted" class="order-deleted">Đơn
										hàng đã hủy</a>
								</div>
							</div>
						</div>
